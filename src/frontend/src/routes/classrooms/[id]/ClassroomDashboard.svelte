<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (classId && role === "teacher") {
            ...
            for (let i = 0; i < conversationResp.conversations.length; i++) {
                ...
                let author = null;
                if (authorData)
                    author = await apiRequest(authorData.sender, "GET");
                ...
                let lastUpdate = "";
                if (lastMessageUrl)
                    lastUpdate = await apiRequest(lastMessageUrl, "GET");
                ...
                conversations.push({
                    ...
                    update:
                        lastUpdate === ""
                            ? "Unknown"
                            : new Date(lastUpdate.postTime).toLocaleString(), // Last update of conversation, not yet callable
    ...
    async function deleteMemberOrRequest(
        id: string,
        role: string,
        type: string
    ) {
        if (type === "member") {
            ...
            acceptedMembers = acceptedMembers.filter(
                (request) => request.id !== id || request.role !== role
            );
            allAcceptedMembers = [...acceptedMembers];
    ...
    function sortQuestions(type: string) {
        ...
        if (type === "assignment") {
            classroom.conversations = [...classroom.conversations].sort(
                (a, b) => {
...
<main>
    ...
    <div class="content-container">
        ...
        <div class="main-content">
            ...
            <button
                class="back-button"
                on:click={() => routeToItem("classrooms")}
                >&larr; {$currentTranslations.classroom.back}</button
            >

            <!-- Class Header with Join Code on the Right -->
            <div class="class-header">
                <h1>
                    {#if classData}{$currentTranslations.classroom.classroom}: {classData.name}{:else}{$currentTranslations
                            .classroom.loading}...{/if}
                </h1>
                {#if role === "teacher"}
                    <div class="join-code">
            ...
            <div class="tables-container">
                ...
                <section class="table-section">
                    ...
                    <div class="filter-buttons">
                        <button on:click={() => toggleAcceptedRole("teacher")}
                            >{$currentTranslations.classroom.show}
                            {$currentTranslations.classroom.teachers}</button
                        >
                        <button on:click={() => toggleAcceptedRole("student")}
                            >{$currentTranslations.classroom.show}
                            {$currentTranslations.classroom.students}</button
                        >
                        <button on:click={() => toggleAcceptedRole("all")}
                            >{$currentTranslations.classroom.show}
                            {$currentTranslations.classroom.all}</button
                        >
                    </div>
                    <table>
                        <thead>
                    ...
                    <section class="table-section">
                        <div class="table-header">
                            ...
                            <div class="filter-buttons">
                                ...
                                <button
                                    on:click={() =>
                                        togglePendingRole("student")}
                                    >{$currentTranslations.classroom.show}
                                    {$currentTranslations.classroom
                                        .students}</button
                                >
                                <button
                                    on:click={() => togglePendingRole("all")}
                                    >{$currentTranslations.classroom.show}
                                    {$currentTranslations.classroom.all}</button
                                >
                            </div>
