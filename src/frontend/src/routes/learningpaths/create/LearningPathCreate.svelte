<script lang="ts">
    ...
    async function postLearningpath(){
        ...
        if($user.role !== userRole || $user.id !== userId){
            errorMessage = "You are not allowed to create a learningpath as someone else";
            return;
        }
...
<main>
    ...
    <form on:submit|preventDefault={postLearningpath}>
        ...
        <input type="text" id="title" bind:value={title} required placeholder="Type your Title here..." />
        <label for="description">Description</label>
        <textarea id="description" bind:value={description} required placeholder="Type your Description here..." rows=4></textarea>
