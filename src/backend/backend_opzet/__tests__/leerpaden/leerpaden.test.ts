...
describe("leerpaden", (): void => {
  ...
  it("krijg een leerpad", async (): Promise<void> => {
    ...
    const willekeurigLeerpadLink =
      leerpadenArray[Math.floor(Math.random() * leerpadenArray.length)];
    ...
    expect(res.body["inhoud"]).toBe(
      website_base + "/leerpaden/" + id + "/inhoud"
    );
  ...
  it("niet bestaande id", async (): Promise<void> => {
    let id: string =
      "dit leer_pad bestaat niet, behalve als hij deze stomme titel gekozen zou hebben";
    const res: Response = await request(index).get("/leerpaden/" + id);
