...
export async function leerpad(req: Request, res: Response, next: NextFunction) {
  ...
  if (!learningPath)
    throw new ExpressException(404, "learningPath not found", next);
...
export async function leerpad_inhoud(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  const learningObjectsList = learningObjects.map((learningObject) => {
    return {
      ...
      next: learningObject.learning_path_nodes[0].transitions_transitions_nextTolearning_path_nodes
        .map((transition) => {
          if (transition.next_learning_path_node != null)
            return {
