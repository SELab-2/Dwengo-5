...
export async function getClassAssignment(
    req: Request,
    res: Response,
    next: NextFunction
) {
    ...
    const auth2 = await doesTokenBelongToStudentInAssignment(
        assignmentId.data,
        JWToken
    );
...
export async function postClassAssignment(
    ...
    if (!classId.success)
        return throwExpressException(400, "invalid classId", next);
    if (!learningpathLink.success)
        return throwExpressException(400, "invalid learningpathLink", next);
    if (!deadline.success)
        return throwExpressException(400, "invalid deadline", next);
