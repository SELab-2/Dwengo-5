...
export async function postGroupConversation(
    req: Request,
    res: Response,
    next: NextFunction
) {
    ...
    const learningobjectLink = zLearningobjectLink.safeParse(
        req.body.learningobject
    );
    ...
    if (!groupId.success)
        return throwExpressException(400, "invalid groupId", next);
    if (!title.success)
        return throwExpressException(400, "invalid title", next);
    if (!learningobjectLink.success)
